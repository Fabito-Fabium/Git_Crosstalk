...
for i in tqdm(range(len(lmbd_spc))):
    ...
    # HA = la.LinearOperator((Nt * Ne, Nt * Ne), matvec=lambda x: olo.HT(olo.H(x, h), h) + lmbd_spc[i] * olo.DT(olo.D(x)) + olo.H(x) + olo.HT(x) + x,
    #                                                rmatvec=lambda x: olo.HT(olo.H(x, h), h) + lmbd_spc[i] * olo.DT(olo.D(x)) + olo.H(x) + olo.HT(x) + x)

    fest = la.minres(AH, olo.HT(g, h) + g, x0=np.zeros(Nt*Ne), maxiter=200, rtol=1e-20, show=False)[0].reshape((Nt, Ne), )
...
# old
AH = la.LinearOperator((Nt * Ne, Nt * Ne), matvec=lambda x: olo.HT(olo.H(x, h), h) + lmbd * olo.D(olo.DT(x)))
#
# HA = la.LinearOperator((Nt * Ne, Nt * Ne), matvec=lambda x: olo.HT(olo.H(x, h), h) + lmbd_spc[i] * olo.DT(olo.D(x)) + olo.H(x) + olo.HT(x) + x,
#                                                    rmatvec=lambda x: olo.HT(olo.H(x, h), h) + lmbd_spc[i] * olo.DT(olo.D(x)) + olo.H(x) + olo.HT(x) + x)

fest, conv = la.minres(AH, olo.HT(g, h) + g,x0=np.zeros(Nt*Ne), maxiter=200, rtol=1e-20, show=True)
fest = fest.reshape((Nt, Ne), )  # 9 s
...
for i in tqdm(range(len(lmbd_spc))):
    ...
    fh = la.minres(AH, olo.HT(crs), x0=fest.ravel(), maxiter=200, rtol=1e-20, show=False)[0].reshape(Nt, Ne)
...
plt.xlabel("$\lambda$")
plt.ylabel("$||f-f_{est}||_2$")
plt.title(f"Lambda minimize, melhor lmbd: {lmbd_spc[np.argmin(norm_res)]}")
...
axs[0].plot(f[:, 3])
# axs[0].set_title(f"f el 1, minres runtime:{t0} seconds")
axs[1].plot(fest[:, 3])
