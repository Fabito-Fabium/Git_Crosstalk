...
def hest_wrong_delay():
    ...
    for i in range(Nc):
        ...
        hest[abs(min(Nh - 1, np.abs(d) - 1) + np.random.randint(-1, 1)), i] = fetch().gain()[np.abs(d)]
...
lmbd_par = Parallel(n_jobs=5)(delayed(get_lmbd_sub_f)(i) for i in range(len(lmbd_f_spc)))
